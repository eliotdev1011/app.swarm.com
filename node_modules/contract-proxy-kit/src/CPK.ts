...
export interface CPKConfig {
  ...
  transactionManager?: CpkTransactionManager
  ownerAccount?: string
...
class CPK {
   ...
   */
  setTransactionManager(transactionManager: CpkTransactionManager): void {
    if (this.#safeAppsSdkConnector?.isSafeApp) {
  ...
  async getExecTransactionsGasLimit(
    transactions: Transaction[],
    options?: ExecOptions
    ...
    if (this.#safeAppsSdkConnector?.isSafeApp && transactions.length >= 1) {
      throw new Error("Can't estimate gas limit in SafeApps")
    }
    ...
    return txManager.getExecTransactionsTransaction({
      ownerAccount,
      safeExecTxParams,
      transactions: standardizedTxs,
      ethLibAdapter: this.#ethLibAdapter,
