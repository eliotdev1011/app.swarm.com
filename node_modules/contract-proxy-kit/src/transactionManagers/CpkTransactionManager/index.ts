...
class CpkTransactionManager implements TransactionManager {
  ...
  async execTransactions({
    ...
    const { txObj, gasLimit } = await this.getExecTransactionsTransaction({
      ownerAccount,
      safeExecTxParams,
      transactions,
      ethLibAdapter,
  ...
  /**
   * Computes the transaction required to executes a list of transactions and its gas limit estimate
   *
   * @param options
   * @returns The transaction object and its gas limit estimate
   */
  async getExecTransactionsTransaction({
    ownerAccount,
    safeExecTxParams,
    transactions,
    ethLibAdapter,
    ...
    isDeployed,
    sendOptions
  }: Omit<ExecTransactionProps, 'isConnectedToSafe'>): Promise<{
    txObj: ContractTxObj
    gasLimit: number
  }> {
    const {
